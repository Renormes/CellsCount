...
def submit():
    try:
        ...
        nome = 'contagens/celulas_'+entry0.get()+'.xlsx'
        ...
        with ThreadPoolExecutor() as executor:
            for filenamer in os.listdir("quadrantes/"):
                img_pil = Image.open("quadrantes/"+filenamer)

                process_image(model, img_pil)
...
def process_image(model, img_pil):
    ...
    for dirname in os.listdir("cutQuadrantes/"):
        crop_path = "cutQuadrantes/"+dirname+"/crops/"
        for dire in os.listdir(crop_path):
            ...
            for filename in os.listdir(dire_path):
                ...
                if image is None:
                    print("Error: Could not open image at "+image_p)
                    exit(1)
                ...
                for row in range(4):
                    for col in range(4):
                        ...
                        sub_image_path = os.path.join("cortes/"+dire, sub_image_filename)
